...
def integral(mean, n=16, truncate_classes=None):
    """Compute multivariate density in the orthant > 0 with covariance matrix equal to I + 1.
    ...
    if truncate_classes is not None:
        if truncate_classes <= 0:
            raise ValueError("Trucation must be positive, got {}.".format(truncate_classes))
        truncate_classes = min(truncate_classes, dim)
        reminder_classes = dim - truncate_classes
        truncated_mean = mean[:, :truncate_classes]
        integrals, truncated_gradients = genz_integral_impl(truncated_mean, n=n)  # (B), (B, DT).
        gradients = torch.cat([truncated_gradients,
                               torch.zeros(b, reminder_classes, dtype=mean.dtype, device=mean.device)],
                              -1)
    else:
...
class PositiveNormalProb(torch.autograd.Function):
    ...
    """
    @staticmethod
    def forward(self, mean, n=16, truncate_classes=None):
        integrals, gradients = integral(mean, n=n, truncate_classes=truncate_classes)
    ...
    def backward(self, grad_output):
        ...
        return gradients * grad_output.unsqueeze(-1), None, None
