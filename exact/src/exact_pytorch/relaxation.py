...
class BetaBernoulliLoss(torch.nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.hidden_weight = torch.nn.Parameter(torch.zeros([]))
...
class Relaxed01Loss(torch.nn.Module):
    """Different approximations of the 0-1 loss.

    Most methods of 0-1 relaxation solve binary classification.
    We apply the same relaxation to all differences between true logit and alternative logits.
    """
