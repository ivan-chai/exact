...
class TestCriterion(TestCase):
    ...
    def test_truncation(self):
        logits1 = torch.tensor([[5, 2, 1]]).float()
        logits2 = torch.tensor([[5, 2]]).float()
        labels = torch.tensor([1])
        criterion = EXACTLoss()
        self.assertNotAlmostEqual(criterion(logits1, labels).item(), criterion(logits2, labels).item())
        criterion = EXACTLoss(truncate_classes=1)
        self.assertAlmostEqual(criterion(logits1, labels).item(), criterion(logits2, labels).item())
