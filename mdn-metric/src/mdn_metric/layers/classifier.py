...
from ..third_party.hyperbolic.hypnn import HyperbolicMLR, ToPoincare
from .bayes import LinearBayes
from .distribution import GMMDistribution, VMFDistribution, DiracDistribution
from .parametrization import Parametrization
from .scorer import FeatureMatchingScorer, HyperbolicScorer
...
def revert_class_grouping(grouped_predictions, indices):
    """Inverse of group_by_class."""
    if grouped_predictions.ndim != 3:
        raise ValueError("Expected grouped tensor with shape (B // L, L, P).")
    if indices.ndim != 2:
        raise ValueError("Expected indices with shape (B // L, L).")
    unsorted_predictions = grouped_predictions.reshape(-1, grouped_predictions.shape[-1])  # (B, L).
    predictions = unsorted_predictions.take_along_dim(torch.argsort(indices.flatten()).reshape(-1, 1), 0)  # (B, L).
    return predictions
...
class LinearClassifier(torch.nn.Linear):
    ...
    @staticmethod
    def get_default_config(use_bias=True, initial_scale=1,
        ...
        Args:
            ...
            normalize_weights: Normalize weights rows before applying.
    ...
    def __init__(self, distribution, num_classes, *, is_multilabel=False, priors=None, config=None):
        ...
        if self._config["use_variance"]:
            self._variance_parametrization = Parametrization(config={"type": self._config["variance_parametrization"],
...
class LinearBayesClassifier(LinearBayes):
    """Simple classification head based on Bayesian linear layer.
    ...
    @staticmethod
    def get_default_config(use_bias=True, **bayes_params):
        """Get classifier config.

        Args:
            use_bias: Whether to use bias in linear layer or not.
            bayes_params: Parameters of the Bayesian layer.
        """
        config = LinearBayes.get_default_config(**bayes_params)
    ...
    def __init__(self, distribution, num_classes, *, is_multilabel=False, priors=None, config=None):
        config = prepare_config(self, config)
        super().__init__(distribution.dim, num_classes, bias=config["use_bias"], config=config)
        self._config = config
        self._distribution = distribution
        self._num_classes = num_classes
...
class ArcFaceClassifier(ArcFace):
    ...
    @staticmethod
    def get_default_config(scale=64.0, margin=0.5):
        """Get classifier config.
    ...
    def forward(self, parameters, labels=None, scorer=None):
        ...
        if self._config["target_distribution"] is None:
            ...
            if self.training:
                ...
                embeddings = self._distribution.mean(parameters)  # (..., D).
            logits = self._target_distribution.logpdf(targets, embeddings.unsqueeze(-2))  # (..., C).
...
class VMFClassifier(torch.nn.Module):
    ...
    def __init__(self, distribution, num_classes, *, is_multilabel=False, priors=None, config=None):
        ...
        components = distribution._config["components"]
        ...
        if self._config["deterministic_target"]:
            ...
            means = torch.randn(num_classes, components, dim) * l / (1 - l * l) * (dim - 1) / math.sqrt(dim)
            self.weight = torch.nn.Parameter(distribution.join_parameters(
                log_probs=torch.zeros(num_classes, components),
                means=means,
...
class SPEClassifier(torch.nn.Module):
    ...
    def __init__(self, distribution, num_classes, *, is_multilabel=False, priors=None, config=None):
        if not isinstance(distribution, GMMDistribution):
    ...
    def forward(self, parameters, labels=None, scorer=None):
        ...
        if labels.shape != parameters.shape[:-1]:
            raise ValueError("Parameters and labels shape mismatch: {}, {}".format(
                parameters.shape, labels.shape))
        by_class, order, label_map = group_by_class(parameters, labels)  # (B, L', P), (B, L'), (L').
        k = len(by_class) // 2
        ...
        all_logits = revert_class_grouping(all_logits, order)
        return all_logits
...
class CentroidFreeFMSClassifier(torch.nn.Module):
    """Classify using scores.
    ...
    @staticmethod
    def get_default_config(use_priors=False, scorer_params=None, match_self=True, cache_size=1):
        """Get classifier config.

        Args:
            use_priors: Use trainset class priors for posterior estimation.
            scorer_params: FMS parameters.
            match_self: Include each element to it's label distribution.
            cache_size: The number of samples per each label stored for inference.
        """
        return OrderedDict([
    ...
    def __init__(self, distribution, num_classes, *, is_multilabel=False, priors=None, config=None):
        ...
        self._distribution = distribution
        self._num_classes = num_classes
        self.scorer = FeatureMatchingScorer(distribution, config=self._config["scorer_params"])
        if (priors is not None) and self._config["use_priors"]:
            self._log_priors = torch.tensor(priors).log()
        else:
            self._log_priors = None
        if self._config["cache_size"] != 1:
            raise NotImplementedError("Mixtures are not supported.")
        self.register_buffer("cache_counts", torch.zeros(num_classes, dtype=torch.long))
        self.register_buffer("cache", torch.zeros(num_classes, distribution.num_parameters))
    ...
    def forward(self, parameters, labels=None, scorer=None):
        scorer = None  # Use self.scorer.
        if self.training:
            by_class, order, label_map = group_by_class(parameters, labels)  # (B', L', P), (B', L'), (L').
            if len(by_class) < 2:
                raise RuntimeError("Need 2 or more samples per class.")
            ubm_components_parameters = parameters.reshape(-1, parameters.shape[-1])  # (N, P).
            ubm_components_log_probs = torch.zeros_like(ubm_components_parameters[:, 0])  # (N).
            ubm_distribution, ubm_parameters = self._distribution.make_mixture(ubm_components_log_probs, ubm_components_parameters)  # (UP, P).
            # Store the first element for each label as label prototype for inference.
            indices = label_map[:, None].tile(1, self._distribution.num_parameters)  # (L', P).
            self.cache_counts.scatter_add_(0, label_map, torch.ones_like(label_map))
            self.cache.scatter_(0, indices, by_class[0])  # (B, L).
        else:
            mask = self.cache_counts > 0
            cache = self.cache[mask]  # (L', P).
            by_class = cache[None, :, :]  # (B, L', P).
            label_map = torch.arange(self._num_classes, device=parameters.device)[mask]  # (L').
            ubm_components_log_probs = torch.zeros_like(cache[:, 0])  # (C).
            ubm_distribution, ubm_parameters = self._distribution.make_mixture(ubm_components_log_probs, cache)
        with self.scorer.set_ubm(ubm_distribution, ubm_parameters):
            log_fms = self.scorer(parameters[:, None, None, :], by_class[None, :, :, :])  # (B, B', L').
        if self.training and (not self._config["match_self"]):
            mask = 1 - torch.eye(len(log_fms), dtype=parameters.dtype, device=parameters.device)  # Not self.
            mask = mask.take_along_dim(order.flatten()[None], 1)  # (B, B).
            mask = mask.reshape(*log_fms.shape)  # (B, B', L').
        else:
            mask = torch.ones_like(log_fms)
        logits = (log_fms * mask + self.LOG_EPS * (1 - mask)).logsumexp(dim=1) - mask.sum(dim=1).log()  # (B, L').
        all_logits = torch.full([logits.shape[0], self._num_classes], self.LOG_EPS,
                                device=logits.device, dtype=logits.dtype)  # (B, L).
        indices = label_map[None, :].tile(logits.shape[0], 1)  # (B, L').
        all_logits.scatter_(1, indices, logits)  # (B, L).
        log_priors = self._log_priors
        if log_priors is None:
            log_priors = -math.log(len(label_map))
        all_logits += log_priors
        return all_logits
...
class BatchSoftmaxClassifier(torch.nn.Linear):
    """Apply softmax along batch dimension to negative L2 scores.

    Note, that inference depends on the batch size.

    Args:
        distribution: Distribution used in the model.
        num_classes: Number of output classes.
        priors: Precomputed class priors.

    Inputs:
        - parameters: Distribution parameters with shape (..., K).
        - labels: Required for inverted classification.
        - scorer: Unused.
    ...
    @staticmethod
    def get_default_config(priors="pretrained", inverted=False, group_inverted=False,
                           momentum=0.1, normalize_probabilities=False,
                           xent_weight=0.0):
        """Get classifier config.

        Args:
            priors: Type of prior correction used (one of `pretrained` or `none` or `fixed`).
                Priors are unused in inverted classification.
            inverted: Solve inverted classification (predict batch element by label).
            group_inverted: Group batch per-label for inverted classification.
            momentum: Momentum used for prior statistics aggregation.
            normalize_probabilities: Divide predicted probabilities by maximum during training.
            xent_weight: Add traditional softmax-crossentropy with some weight.
        """
        return OrderedDict([
            ("priors", priors),
            ("inverted", inverted),
    ...
    def __init__(self, distribution, num_classes, *, is_multilabel=False, priors=None, config=None):
        config = prepare_config(self, config)
        use_bias = (not config["inverted"]) and (config["priors"] in ("pretrained", "fixed"))
        super().__init__(distribution.dim, num_classes, bias=use_bias)
        self._config = config
        self._distribution = distribution
        self._num_classes = num_classes
        self._is_multilabel = is_multilabel
        self.register_buffer("log_avg_probs", torch.full((num_classes,), -math.log(num_classes)))
        if not config["inverted"]:
            if config["priors"] == "pretrained":
                with torch.no_grad():
                    self.bias.data.copy_(get_log_priors(num_classes, priors))
            elif config["priors"] == "fixed":
                with torch.no_grad():
                    log_priors = get_log_priors(num_classes, priors)
                    if self._is_multilabel:
                        self.bias.data.copy_(-torch.log(1 / torch.exp(log_priors) - 1))
                    else:
                        self.bias.data.copy_(log_priors)
                    self.bias.requires_grad = False
            else:
                if config["priors"] != "none":
                    raise ConfigError("Unknown priors type: {}".format(config["priors"]))
        else:
            if is_multilabel:
                raise ValueError("Can't apply inverted classification in multi-label setup.")
    ...
    def _compute_logits(self, log_pdfs):
        assert log_pdfs.ndim == 3, "Expected log_pdfs with shape (*, B, C)."
        dim = -2  # Apply batch softmax along this dimension.
        batch_size = log_pdfs.shape[dim]
        # Eval log_priors with shape (C) or scalar.
        if self._config["inverted"]:
            log_priors = -math.log(batch_size)
        elif self._is_multilabel:
            log_priors = torch.nn.functional.logsigmoid(self.bias) if self.has_bias else -math.log(2)
        else:
            log_priors = self.bias - torch.logsumexp(self.bias, 0) if self.has_bias else -math.log(self._num_classes)
        if self.training:
            embeddings_log_priors = torch.logsumexp(log_pdfs, dim=dim) - math.log(batch_size)  # (*, C).
            with torch.no_grad():
                self.log_avg_probs.copy_(torch.logaddexp(
                    self.log_avg_probs + math.log(1 - self._config["momentum"]),
                    embeddings_log_priors.logsumexp(dim=0) - math.log(embeddings_log_priors.shape[0]) + math.log(self._config["momentum"])
                ))
        else:
            embeddings_log_priors = self.log_avg_probs[None]  # (1, C).
        logits = log_priors - embeddings_log_priors.unsqueeze(dim) + log_pdfs  # (*, B, C).
        if self.training and self._config["normalize_probabilities"]:
            logits_max = logits.max()
            if logits_max.item() > 0:
                logits = logits - logits_max
        return logits  # (*, B, C).

    def forward(self, parameters, labels=None, scorer=None):
        prefix = tuple(parameters.shape[:-1])
        parameters = parameters.reshape(-1, self._distribution.num_parameters)  # (*, P).
        log_pdfs = self._distribution.logpdf(parameters[:, None, :], self.weight[None, :, :])  # (*, C).
        if self.training and self._config["inverted"] and self._config["group_inverted"]:
            if labels is None:
                raise ValueError("Need labels for inverted training.")
            by_class, order, _ = group_by_class(log_pdfs, labels)  # (B, L', C), (B, L').
            logits = self._compute_logits(by_class)  # (B, L', C).
            logits = revert_class_grouping(logits, order)  # (B, C).
        else:
            logits = self._compute_logits(log_pdfs[None])[0]
        if self._config["xent_weight"] != 0:
            xent_logits = torch.nn.functional.log_softmax(log_pdfs, dim=-1)  # (B, C).
            logits = logits + self._config["xent_weight"] * xent_logits
        return logits.reshape(*(prefix + (self._num_classes,)))
...
class HyperbolicClassifier(torch.nn.Module):
    """Hyperbolic softmax classifier. https://arxiv.org/pdf/1805.09112.pdf
    ...
    Inputs:
        - parameters: Distribution parameters with shape (..., K).
        - labels: Unused.
        - scorer: Scorer used for embedding comparison.
    ...
    def __init__(self, distribution, num_classes, *, is_multilabel=False, priors=None, config=None):
        ...
        if config is not None:
            raise ValueError("No config for HyperbolicClassifier.")
        self._config = config
        self._distribution = distribution
        self._num_classes = num_classes
        self._hyp_mlr = HyperbolicMLR(distribution.dim, num_classes, 1.0)
        self._expmap = ToPoincare(1.0)
    ...
    def forward(self, parameters, labels=None, scorer=None):
        if isinstance(scorer, HyperbolicScorer):
            self._hyp_mlr.c = scorer._config["curvature"]
            self._expmap.c = scorer._config["curvature"]
            self._expmap.riemannian.c = scorer._config["curvature"]
        else:
            raise ValueError("Expected HyperbolicScorer for HyperbolicClassifier.")
        if self.training:
            embeddings, _ = self._distribution.sample(parameters)  # (..., D).
        else:
            embeddings = self._distribution.mean(parameters)  # (..., D).
        logits = self._hyp_mlr(self._expmap(embeddings))
        return logits
