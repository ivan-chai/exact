...
from mdn_metric.config import prepare_config, update_config
...
class LogIvFunction(torch.autograd.Function):
    ...
    def forward(ctx, v, z):
        ...
        z_numpy = z.cpu().double().numpy()
        ive = torch.from_numpy(scipy.special.ive(v, z_numpy)).to(z.dtype).to(z.device)
        ctx.saved_v = v
        ctx.saved_z = z_numpy
    ...
    def backward(ctx, grad_output):
        ...
        ive_shifted = torch.from_numpy(scipy.special.ive(v + 1, z_numpy)).to(grad_output.dtype).to(grad_output.device)
...
class VMFDistribution(DistributionBase):
    ...
    @staticmethod
    def get_default_config(dim=512, components=1, k="separate",
                           parametrization_params=None, max_logk=10,
                           logiv_type="default"):
        """Get vMF parameters.
        ...
        Args:
            ...
            parameterization_params: Concentration parametrization parameters.
    ...
    def __init__(self, config=None):
        ...
        self._parametrization = Parametrization(
            config=update_config(self._config["parametrization_params"], {"min": min_ik}, no_overwrite=True))
    ...
    def num_parameters(self):
        ...
        return prob_parameters + self._config["components"] * (mean_parameters + k_parameters)
    ...
    def make_normalizer(self):
        ...
        if self._config["k"] == K_NORM:
            ...
            normalizer = BatchNormNormalizer(self.num_parameters, begin=means_offset, end=means_offset + c * dim)
        return normalizer
    ...
    def split_parameters(self, parameters, normalize=True):
        ...
        if c > 1:
            ...
            means_offset = 0
        scaled_means = parameters[..., means_offset:means_offset + c * dim].reshape(*(dim_prefix + [c, dim]))
        if isinstance(self._config["k"], Number):
            ik = torch.full((dim_prefix + [c, 1]), 1 / self._config["k"], dtype=parameters.dtype, device=parameters.device)
            hidden_ik = self._parametrization.ipositive(ik)
        elif self._config["k"] == K_SEPARATE:
            hidden_ik = parameters[..., means_offset + c * dim:].reshape(*(dim_prefix + [c, 1]))
        else:
    ...
    def estimate(self, sample):
        ...
        NOTE: Approximation is implemented. See "Parameter Estimation for von Mises–Fisher
        Mixture Model via Gaussian Distribution" (2014) for more details.
        ...
        if isinstance(self._config["k"], Number):
            k = torch.full_like(mean[..., :1], self._config["k"])
        else:
            k = (1 - 1 / self.dim) / sample.var(dim=-2, keepdim=True, unbiased=False).mean(dim=-1, keepdim=True)  # (*, 1, 1).
        log_probs = torch.zeros_like(mean[..., 0])
        return self.pack_parameters({"log_probs": log_probs, "mean": mean, "k": k})  # (*, P).
    ...
    def sample(self, parameters, size=None):
        ...
        hidden_ik = hidden_ik.broadcast_to(list(size) + [c, 1])
    ...
    def mean(self, parameters):
        ...
        if self._config["components"] == 1:
            ...
            means = (component_means * log_probs[..., None].exp()).sum(-2)  # (..., D).
        return means
    ...
    def prior_kld(self, parameters):
        ...
        log_priors, means, hidden_ik = self.split_parameters(parameters)  # (..., C), (..., C, D), (..., C, 1).
        if self._config["components"] != 1:
            raise NotImplementedError("KL divergence with for the mixture.")
    ...
    def pdf_product(self, parameters1, parameters2):
        ...
        new_parameters = self.join_parameters(
            ...
            new_hidden_ik.reshape(*(prefix + (c2, -1)))
        )  # (..., P).
