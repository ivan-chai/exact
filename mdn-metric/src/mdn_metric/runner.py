...
class Runner(dl.IRunner):
    ...
    Args:
        ...
        device: Overwrite default device.
    ...
    def get_default_config(dataset_params=None,
        ...
        Args:
            ...
            hopt_backend: Type of hyperparameter search algorithm ("wandb-grid", "wandb-random", "wandb-bayes" or "optuna-tpe").
    ...
    def get_engine(self):
        device = self._device if self._device is not None else get_default_device()
        if not device.lower().startswith("cuda"):
            return dl.DeviceEngine(device)
    ...
    def get_model(self, stage):
        ...
        model = Model(self._datasets.num_train_classes,
                      is_multilabel=self._datasets.is_multilabel,
        ...
        if checkpoint_path is not None:
            ...
            if training and self._config["resume_prefixes"]:
                ...
                if self._no_strict_init:
                    for k, v in model.state_dict().items():
                        if checkpoint.get(k, v).shape != v.shape:
                            print("Exclude parameter {} from checkpoint due to shape mismatch.".format(k))
                            checkpoint.pop(k)
    ...
    def get_callbacks(self, stage):
        ...
        if stage != self.STAGE_TEST:
            ...
            if model.prior_kld is not None:
                criterion_inputs["weights_prior_kld"] = "weights_prior_kld"
        ...
        for name, dataset in datasets.items():
            ...
            if dataset.classification:
                dataset_callbacks = self._metrics.get_classification_callbacks(
                    ...
                    embeddings_key="aggregated_embeddings" + suffix,
                    final_weights_key="final_weights" + suffix if model.has_final_weights else None,
                    logits_key="aggregated_logits" + suffix if model.classification else None,
    ...
    def handle_batch(self, batch):
        ...
        model = get_base_module(self.model["model"])
        weights_prior_kld = model.prior_kld
        if weights_prior_kld is not None:
            batch["weights_prior_kld"] = weights_prior_kld
            metrics["weights_prior_kld"] = weights_prior_kld.item()
    ...
    def _handle_classification_batch(self, batch):
        ...
        batch["embeddings"] = results["distributions"]  # (B, N, D).
        batch["aggregated_embeddings"] = batch["embeddings"].mean(1)  # (B, D).

        model = get_base_module(self.model["model"])
        if model.classification:
            batch["logits"] = results["logits"]  # (B, N, C).
            batch["aggregated_logits"] = results["logits"].mean(1)  # (B, C).
            if model.has_final_weights:
        ...
        if model.distribution.has_confidences:
            batch["confidences"] = model.distribution.confidences(batch["aggregated_embeddings"])  # (B).
    ...
    def _handle_verification_batch(self, batch):
        batch["embeddings1"] = self.model["embedder"](batch["images1"])  # (B, N, D).
        batch["embeddings2"] = self.model["embedder"](batch["images2"])  # (B, N, D).
        batch["aggregated_embeddings1"] = batch["embeddings1"].mean(1)  # (B, D).
        batch["aggregated_embeddings2"] = batch["embeddings2"].mean(1)  # (B, D).
        batch["scores"] = self.model["scorer"](batch["aggregated_embeddings1"], batch["aggregated_embeddings2"])  # (B).
        model = get_base_module(self.model["model"])
        if model.distribution.has_confidences:
            confidences1 = model.distribution.confidences(batch["aggregated_embeddings1"])  # (B).
            confidences2 = model.distribution.confidences(batch["aggregated_embeddings2"])  # (B).
