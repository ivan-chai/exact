...
def train(args):
    ...
    runner = Runner(args.train_root, args.data,
                    config=args.config, logger=args.logger, device=args.device,
                    initial_checkpoint=args.checkpoint, no_strict_init=args.no_strict_init,
    ...
    if (args.from_stage or 0) >= 0:
        if args.config is not None:
            ...
            with open(args.config) as fp:
                print(fp.read())
        metrics.update(runner.train(verbose=True))
