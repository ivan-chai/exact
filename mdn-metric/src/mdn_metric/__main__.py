...
def parse_arguments():
    ...
    parser.add_argument("--device", help="Use specified device.")
...
@contextmanager
def ProbablyTemporaryDirectory(root=None):
    if root is not None:
        yield root
    else:
        with tempfile.TemporaryDirectory() as root:
            print("Created temporary training root at {}".format(root))
            yield root
...
if __name__ == "__main__":
    ...
    with ProbablyTemporaryDirectory(args.train_root) as root:
