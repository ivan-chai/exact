    ...
    for k, v in config.items():
        if k == CONFIG_HOPT:
            for hk, hv in v.items():
                hopt[hk] = hv
        elif isinstance(v, config_classes):
            for sk, sv in as_flat_config(v, flatten_lists=flatten_lists).items():
                if sk == CONFIG_HOPT:
...
def update_config(config, patch, no_overwrite=False, recurse_lists=False):
    """Merge patch into config recursively."""
    if patch is None:
        return config
    config = get_config(config)
    flat = as_flat_config(config, flatten_lists=recurse_lists)
    flat_patch = as_flat_config(patch, flatten_lists=recurse_lists)
    hopt = flat.pop(CONFIG_HOPT, {})
    hopt_patch = flat_patch.pop(CONFIG_HOPT, {})
    if no_overwrite:
        if set(flat_patch) & set(flat):
            raise ValueError("Patch overwrites config.")
        if set(hopt_patch) & set(hopt):
            raise ValueError("Patch overwrites HOPT config.")
    flat.update(flat_patch)
    hopt.update(hopt_patch)
    if hopt:
        flat[CONFIG_HOPT] = hopt
