...
from .cifar import CIFAR10Dataset, CIFAR100Dataset, CIFAR10SplitClassesDataset, CIFAR100SplitClassesDataset
...
from .transform import TransformDataset, RepeatDataset, PreloadDataset,\
    SamplePairsDataset, UniformLabelNoiseDataset, TailIdentityLabelNoiseDataset
from .transform import LossyDataset, MergedDataset, ClassMergedDataset, ImbalancedDataset
...
class DatasetCollection:
    ...
    DEVSETS = {
        ...
        "cifar10-openset": CIFAR10SplitClassesDataset,
        "cifar100-openset": CIFAR100SplitClassesDataset,
        ...
        "mnist": MnistDataset,
        "svhn": SVHNDataset,
        "pascal-voc": lambda root: PASCALVOCDataset(root, split="train"),
        "ms-coco": lambda root: MSCOCODataset(root, split="train"),
        ...
        "debug-multi-openset": lambda root: DebugDataset(root, multilabel=True)
    }

    VALSETS = {
        "flower102": lambda root: Flower102Dataset(root, annotation_key="valid"),
        "imagenetlt": lambda root: ImageNetLTDataset(root, mode="val"),
        "pascal-voc": lambda root: PASCALVOCDataset(root, split="val"),
    }
    ...
    TESTSETS = {
        ...
        "cifar10-openset": lambda root: CIFAR10SplitClassesDataset(root, train=False),
        "cifar100-openset": lambda root: CIFAR100SplitClassesDataset(root, train=False),
        ...
        "pascal-voc": lambda root: PASCALVOCDataset(root, split="test"),
        "ms-coco": lambda root: MSCOCODataset(root, split="val"),
        ...
        "debug-multi-openset": lambda root: DebugDataset(root, train=False, multilabel=True)
    }
    ...
    def get_default_config(name=None,
                           ...
                           mixup_type=None,
                           batch_size=256, samples_per_class=4, uniform_sampling=False, subset_classes=1,
                           num_workers=8, num_valid_workers=None, persistent_workers=False,
                           shuffle_train=True, train_repeat=1, preload=False,
                           use_lossy_trainset=False, add_lossy_valsets=False, add_lossy_testsets=False, lossy_params=None,
                           add_verification_valsets=False, add_verification_testsets=True, validate_on_test=False, validate_on_train=False,
                           uniform_label_noise_fraction=0.0, small_identity_label_noise_fraction=0.0,
                           large_identity_label_noise_fraction=0.0, preserve_non_tail_classes=False,
                           add_class_imbalance=False, imbalanced_params=None):
        ...
        Args:
            ...
            subset_classes: Use a fraction of classes during training. Must be in the range [0, 1].
                Available only if samples_per_class is not None.
            ...
            use_lossy_trainset: Use lossy variant of the training set.
            ...
            validate_on_train: Compute train metrics between epochs.
            uniform_label_noise_fraction: A fraction of samples in dataset to noise.
            small_identity_label_noise_fraction: A fraction of small ids samples in dataset
                to noise with TailIdentityLabelNoiseDataset.
            large_identity_label_noise_fraction: A fraction of large ids samples in dataset
                to noise with TailIdentityLabelNoiseDataset.
            preserve_non_tail_classes: Whether to use labels of non-tail classes when noising tail classes.
            add_class_imbalance: Whether to resample dataset for imbalance.
            imbalanced_params: Parameters of ImbalancedDataset class.
        """
        return OrderedDict([
            ("name", name),
            ...
            ("preload", preload),
            ("use_lossy_trainset", use_lossy_trainset),
            ...
            ("uniform_label_noise_fraction", uniform_label_noise_fraction),
            ("small_identity_label_noise_fraction", small_identity_label_noise_fraction),
            ("large_identity_label_noise_fraction", large_identity_label_noise_fraction),
            ("preserve_non_tail_classes", preserve_non_tail_classes),
            ("add_class_imbalance", add_class_imbalance),
            ("imbalanced_params", imbalanced_params)
        ])
    ...
    def __init__(self, data_root, *, config):
        ...
        # Cache datasets.
        self.get_trainset = lru_cache()(self.get_trainset)
        self.get_valsets = lru_cache()(self.get_valsets)
        self.get_testsets = lru_cache()(self.get_testsets)

        # Extract trainset properties.
        trainset = self.get_trainset(transform=False)
        self._num_classes = trainset.num_classes
        self.is_multilabel = trainset.multilabel
    ...
    def get_trainset(self, transform=True, augment=True):
        ...
        if self._config["add_class_imbalance"]:
            dataset = ImbalancedDataset(dataset, **self._config["imbalanced_params"])

        if sum([self._config["uniform_label_noise_fraction"] > 0.0,
                self._config["small_identity_label_noise_fraction"] > 0.0,
                self._config["large_identity_label_noise_fraction"] > 0.0]) > 1:
            raise ValueError("Only one dataset noise mode can be selected.")

        if self._config["uniform_label_noise_fraction"] > 0.0:
            dataset = UniformLabelNoiseDataset(dataset, self._config["uniform_label_noise_fraction"])
        if self._config["small_identity_label_noise_fraction"] > 0.0:
            dataset = TailIdentityLabelNoiseDataset(dataset, self._config["small_identity_label_noise_fraction"],
                                                    noise_large_ids=False,
                                                    preserve_non_tail_classes=self._config["preserve_non_tail_classes"])
        if self._config["large_identity_label_noise_fraction"] > 0.0:
            dataset = TailIdentityLabelNoiseDataset(dataset, self._config["large_identity_label_noise_fraction"],
                                                    noise_large_ids=True,
                                                    preserve_non_tail_classes=self._config["preserve_non_tail_classes"])
        if self._config["use_lossy_trainset"]:
            dataset = LossyDataset(dataset, config=self._config["lossy_params"])
    ...
    def get_valsets(self, transform=True):
        ...
        if self._config["validate_on_train"]:
            train_eval_dataset = self.DEVSETS[self._config["name"]](self._data_root)
            base_valsets["valid-train"] = train_eval_dataset
    ...
    def _get_loader(self, dataset, train):
        ...
        if train and dataset.classification and (self._config["samples_per_class"] is not None):
            ...
            kwargs["batch_sampler"] = ShuffledClassBalancedBatchSampler(dataset,
                                                                        ...
                                                                        subset_classes=self._config["subset_classes"])
        else:
