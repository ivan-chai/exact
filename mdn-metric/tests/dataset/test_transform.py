...
from mdn_metric.dataset.common import Dataset
from mdn_metric.dataset.transform import *
...
class TestTransform(TestCase):
    ...
    def test_imbalanced(self):
        ...
        imbalanced_dataset = ImbalancedDataset(base_dataset, imbalance_ratio=0.1, distribution_type="step")
        self.assertEqual(len(imbalanced_dataset.class_sizes), len(np.unique(base_dataset.labels)))
        self.assertEqual(imbalanced_dataset.class_sizes[-1] / imbalanced_dataset.class_sizes[0], 0.1)
        self.assertEqual(len(np.unique(np.array(imbalanced_dataset.class_sizes))), 2)
        self.assertTrue(all(np.array(imbalanced_dataset.class_sizes) ==
                            sorted(np.unique(imbalanced_dataset.labels, return_counts=True)[1])[::-1]))

        # Test `exp` distribution.
        imbalanced_dataset = ImbalancedDataset(base_dataset, imbalance_ratio=0.1, distribution_type="exp")
        self.assertEqual(len(imbalanced_dataset.class_sizes), len(np.unique(base_dataset.labels)))
        self.assertEqual(imbalanced_dataset.class_sizes[-1] / imbalanced_dataset.class_sizes[0], 0.1)
        self.assertTrue(all(np.array(imbalanced_dataset.class_sizes) ==
                            sorted(np.unique(imbalanced_dataset.labels, return_counts=True)[1])[::-1]))
        for i in range(len(imbalanced_dataset.class_sizes) - 1):
            self.assertAlmostEqual(imbalanced_dataset.class_sizes[i + 1],
                                   imbalanced_dataset.class_sizes[i] * 0.1 ** (1 / base_dataset.num_classes), delta=1)
