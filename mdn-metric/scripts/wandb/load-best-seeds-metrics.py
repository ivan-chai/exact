...
def parse_arguments():
    ...
    parser.add_argument("--ignore-broken-runs", help="Ignore broken runs.", action="store_true")
...
def get_best_metrics(runs, num_seeds, metric_regexps, selection_metric, selection_maximize, ignore_broken_runs):
    ...
    for run in runs:
        if ignore_broken_runs:
            if selection_metric not in run.summary:
                continue
...
def main(args):
    ...
    metrics = get_best_metrics(runs, args.num_seeds, args.metric_regexp, args.selection_metric, args.selection_maximize,
                               ignore_broken_runs=args.ignore_broken_runs)
